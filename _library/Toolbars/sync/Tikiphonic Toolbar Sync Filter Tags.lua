...
--------------------------------------------
local library_files = {
  'Toolbars/helpers/Tikiphonic Get Filter Tags.lua'
}
...
function SyncTikiPhonicToolbarFilterTags()
  ...
  if (totalTags.isChanged) then -- only do this looping if filter text changed
    ...
    for index, tagname in ipairs(tikiphonic_config.tags) do
      ...
      if (isFound) then
        reaper.SetToggleCommandState(0, reaper.NamedCommandLookup(tikiphonic_config.scriptCommandIDs.tagFilter[tagname]),
          1)
      else
        reaper.SetToggleCommandState(0, reaper.NamedCommandLookup(tikiphonic_config.scriptCommandIDs.tagFilter[tagname]),
          -1)
